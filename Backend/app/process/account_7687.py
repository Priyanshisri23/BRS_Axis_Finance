...
def load_input_files(session, email_to, config_data):
    ...
    transactionsummary_file = config_data['transactionsummary_7687']
    brs_file = config_data['brs_7687']
    ...
    try:
        ...
        transactionsummary_filepath = input_folder /ConfigConstants.SUBFOLDER_7687/ transactionsummary_file
        general_logger.info(f"Loading Transaction Summary 7687 from {transactionsummary_filepath}")
        ...
        if not check_columns_exists(transactionsummary_df, 
                                     ...
                                     'Transaction Summary 7687', session, email_to, transactionsummary_filepath):
            general_logger.error(f"Validation failed for Transaction Summary 7687 (File: {transactionsummary_filepath})")
            return None
        
        dataframes['Transaction Summary 7687'] = transactionsummary_df
        general_logger.info(f"Transaction Summary 7687 validated successfully (File: {transactionsummary_filepath})")

        # Load and validate BRS file
        brs_filepath = input_folder /ConfigConstants.SUBFOLDER_7687/ brs_file
        brs_output_folder= os.path.join(output_folder,ConfigConstants.SUBFOLDER_7687)
        general_logger.info(f"Loading BRS 7687 from {brs_filepath}")
        brs_7687_df = load_file(brs_filepath, headerrow='Date', sheet_name=t_2_date)
        shutil.copy(brs_filepath,brs_output_folder)

        print(brs_7687_df.columns)
        if not check_columns_exists(brs_7687_df, 
                                     ['Particulars', 'Date', 'Amount'], 
                                     'BRS 7687', session, email_to, brs_filepath):
            general_logger.error(f"Validation failed for BRS 7687 (File: {brs_filepath})")
            return None
        
        dataframes['BRS 7687'] = brs_7687_df
        general_logger.info(f"BRS 7687 validated successfully (File: {brs_filepath})")

        insert_detail_log(session, "Axis Finance Bank Reconciliation Process", "Axis Finance BRS Task", "N/A",
                            'BRS.AutoMail@axisfinance.in', f'N/A', f"Created Folder Structure of 7687", loglevel="Info")
...
def generate_brs_report(transaction_summary_df, brs_7687_df, brs_output_file):
    try:
        ...
        filtered_df['Final Remark'] = filtered_df.apply(
            lambda row: 'Debit in Bank Statement - Not in system' if row['Amount'] < 0 else 'Credit in Bank Statement - Not in system',
            axis=1
        )

        # filtered_df['Aging Bucket'] = filtered_df['Aging'].apply(aging_bucket)
        ...
        # # Append new rows to brs_7687_df
        brs_7687_df = pd.concat([brs_7687_df, transaction_filtered])

        previous_date_dt = pd.Timestamp(previousdate)

        # Convert 'Value Date' to datetime and calculate Aging
        brs_7687_df['Date'] = pd.to_datetime(
            brs_7687_df['Date'], format='%d-%m-%Y', errors='coerce', dayfirst=True)

        brs_7687_df['Aging'] = (previous_date_dt - brs_7687_df['Date']).dt.days
        ...
        brs_7687_df['Aging Bucket'] = brs_7687_df['Aging'].apply(aging_bucket)

        # Summing up Amounts
        total_debit_in_bank = brs_7687_df[brs_7687_df['Final Remark'].str.contains('Debit in Bank Statement', na=False,case=False)]['Amount'].sum()
        total_credit_in_bank = brs_7687_df[brs_7687_df['Final Remark'].str.contains('Credit in Bank Statement', na=False,case=False)]['Amount'].sum()
        ...
        data = {
            ...
            'Value': [
                'AXIS FINANCE LIMITED- MORTGAGE DISBURSEMENT A/C',
                '919020012447687',
                None,
        ...
        with pd.ExcelWriter(brs_output_file, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
            ...
            brs_7687_df.to_excel(writer,sheet_name=t_1_date,index=False,startrow=len(df)+3)
        ...
        # handle_critical_error(session, "Axis bank", e, shared_folder, email_to)


def main_account_7687(to_email:str):
    """Main function that runs the BRS process for 7687."""
    try:
        start_mail(to_email,"Axis Finance BRS of Account 7687")


        try:
            sftp_handler_axis.sftp_hander(account_no = 7687)
        except Exception as e:
            print(f"Error occured in SFTP 7687")

        dataframes = load_input_files(session, to_email, config_data)
        brs_file = config_data['brs_7687']

        if dataframes:
            transaction_summary_df = dataframes['Transaction Summary 7687']
            brs_7687_df = dataframes['BRS 7687']
            
            output_folder = Path(ConfigConstants.OUTPUT_FOLDER)
            brs_output_file = os.path.join(output_folder,ConfigConstants.SUBFOLDER_7687, brs_file)

            generate_brs_report(transaction_summary_df, brs_7687_df, brs_output_file)

            color_excel(brs_output_file,t_1_date)
            general_logger.info("BRS process completed successfully.")
        
            insert_detail_log(session, "Axis Finance Bank Reconciliation Process", "Axis Finance BRS Task", "N/A",
                                'BRS.AutoMail@axisfinance.in', f'N/A', f"Working of Account 7687 is done", loglevel="Info")
            
            upload_file_sftp(([brs_output_file]), "7687")

            success_mail(to_email,"Axis Finance BRS",[brs_output_file])

            result = f""" Account 7687 have been processed successfully!!
        ...
        return f"Error in processing BRS for Account - 7687: {e}", "Failure"
